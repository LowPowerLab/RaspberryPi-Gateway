...
exports.events = {
  ...
  sumpSMS : { label:'SumpPump : SMS (below 20cm)', icon:'comment', descr:'Send SMS if water < 20cm below surface', serverExecute:function(node) { if (node.metrics['CM'] && node.metrics['CM'].value < 20 && (Date.now() - new Date(node.metrics['CMD'].updated).getTime() < 2000)) { sendSMS('SUMP PUMP ALERT', 'Water is only 20cm below surface and rising - [' + node._id + '] ' + node.label + ' @ ' + (new Date().toLocaleTimeString() + (new Date().getHours() > 12 ? 'PM':'AM'))); }; } },
  //timer based events
  switchMoteON_PM : { label:'SwitchMote ON at 8PM!', icon:'clock', descr:'Turn this switch ON at 8PM every day', nextSchedule:function(node) { return exports.timeoutOffset(20,0); /*run at 8PM sharp*/ }, scheduledExecute:function(node) { if (node.metrics['B1'] && node.metrics['B1'].value == 'OFF') { sendMessageToNode({nodeId:node._id, action:'BTN1:1'}); }; } },
  switchMoteOFF_AM : { label:'SwitchMote OFF at 6:45AM!', icon:'clock', descr:'Turn this switch OFF at 6:45AM every day', nextSchedule:function(node) { return exports.timeoutOffset(6,45); /*run at 6:45AM */ }, scheduledExecute:function(node) { if (node.metrics['B1'] && node.metrics['B1'].value == 'ON') { sendMessageToNode({nodeId:node._id, action:'BTN1:0'}); }; } },
};
